...
public class Client {
    ...
    public List <Product> getProducts() throws IOException, InterruptedException {
        ...
        HttpRequest request = HttpRequest.newBuilder()
                ...
                .POST(HttpRequest.BodyPublishers.ofString(query)) // sending the query as the body (POST)
                .build();

        // Actually sends it to Vendure and waits for the response. response.body() will contain the JSON with all the products.
        HttpResponse<String> response = client
                .send(request, HttpResponse.BodyHandlers.ofString()); // send the request and demand that the response is in String

        // then parse response.body() to get products
        JSONObject json = new JSONObject(response.body());
        JSONArray items = json
                .getJSONObject("data") // goes to what is contained in data
